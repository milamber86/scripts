...
curl --connect-timeout ${ctimeout} -m ${ctimeout} -ikL "https://${iwserver}/webmail/server/download.php?class=file&fullpath=${calexport}&sid=wm-${wcSid}" > "${targetPath}"
end=`date +%s%N | cut -b1-13`
runtime=$((end-start))
echo "${runtime};${targetPath}";
}

function getFolders # ( 1: wcSid, 2: user@email -> gw folders list )
{
wcSid="${1}";
email="${2}";
start=`date +%s%N | cut -b1-13`
...
runtime=$((end-start))
echo "${getfolder_response}" | egrep -o '\"TYPE\":\[\{\"VALUE\":\"[[:alnum:]]\"\}\]|\"RELATIVE_PATH\":\[\{\"VALUE\":\"[[:alnum:]\\ \/]*\"\}\]' | tr -d '[]{}' | sed -r 's|"VALUE":||' > "${tmpFile}"
}

function parseFolders # ( 1: folder list from getFolders -> folder_name;type )
{
local wantType=0;
